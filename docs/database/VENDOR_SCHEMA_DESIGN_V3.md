# Vendor Schema Design (V3)

**Status:** Approved & Finalized (DoorDash & Uber Eats Parity)  
...
This document defines the production relational schema for merchant organizations, multi-location store operational states, kitchen busy modes, pause mechanisms, fulfillment capabilities, catalog discovery parameters, branding assets, and physical operating locations.

The design incorporates DoorDash Drive & Uber Eats Store API patterns:

- A user belonging to one or more vendors.
- A vendor operating multiple physical locations.
- Vendor-scoped permissions through memberships, with optional location-level scoping for managers and staff (`vendor_membership_locations`).
- Vendor categories, tags, logo/banner media assets, and platform fee tier references.
- Kitchen **Busy Mode** (`busy_mode_prep_padding_minutes`) and **Order Pause** (`paused_until`, `pause_reason`).
- **Fulfillment Capabilities** toggles (`is_delivery_enabled`, `is_pickup_enabled`, `is_curbside_enabled`).
- Separate **Driver vs Customer Handoff Instructions** (`driver_pickup_instructions`, `customer_pickup_instructions`, `parking_instructions`).
- **Kitchen Order Notification Channel** (`order_notification_channel`: `TABLET`, `POS`, `WEBHOOK`, `SMS`, `EMAIL`).
...
- `vendor_locations` (physical stores, PostGIS coordinates, separate driver/customer handoff instructions, parking instructions, website URL, google_place_id, ratings, status)
...
- `vendor_location_commerce_settings` (prep times, kitchen busy padding, order pause, fulfillment toggles, notification channel, fees, tax calculation rules)
...
| `website_url` | `VARCHAR(512)` | Yes | — | Merchant location website |
| `google_place_id` | `VARCHAR(255)` | Yes | — | Google Places API identifier |
...
| `driver_pickup_instructions` | `TEXT` | Yes | — | Instructions specifically for delivery drivers |
| `customer_pickup_instructions` | `TEXT` | Yes | — | Instructions specifically for customer pickup |
| `parking_instructions` | `TEXT` | Yes | — | Designated parking/loading spot rules |
| `created_at` | `TIMESTAMPTZ` | No | `now()` | Creation time |
| `updated_at` | `TIMESTAMPTZ` | No | `now()` | Last update time |
| `version` | `BIGINT` | No | `0` | Optimistic concurrency version |
...
Location-specific operational parameters and DoorDash/Uber Eats kitchen controls.
...
| `paused_until` | `TIMESTAMPTZ` | Yes | — | Order pause auto-resume timestamp |
| `pause_reason` | `VARCHAR(255)` | Yes | — | Reason for pause (`KITCHEN_OVERWHELMED`, etc.) |
| `busy_mode_prep_padding_minutes` | `INTEGER` | No | `0` | Dynamic extra prep time buffer during rush hours |
| `is_delivery_enabled` | `BOOLEAN` | No | `TRUE` | Capability switch for delivery fulfillment |
| `is_pickup_enabled` | `BOOLEAN` | No | `TRUE` | Capability switch for pickup fulfillment |
| `is_curbside_enabled` | `BOOLEAN` | No | `FALSE` | Capability switch for curbside fulfillment |
| `order_notification_channel` | `VARCHAR(32)` | No | `'TABLET'` | Hardware channel (`TABLET`, `POS`, `WEBHOOK`, `SMS`, `EMAIL`) |
...
- [x] DoorDash/Uber Eats kitchen pause, busy mode padding, and notification channels included.
- [x] Fulfillment capabilities (`delivery`, `pickup`, `curbside`) included.
- [x] Separate driver vs customer pickup instructions and parking instructions included.
- [x] Database migration script `V2__create_vendor_schema.sql` updated.
